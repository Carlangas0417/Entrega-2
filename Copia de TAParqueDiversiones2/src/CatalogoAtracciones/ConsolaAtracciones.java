...
public class ConsolaAtracciones extends CGeneral {
    ...
    public void ejecutar() throws IOException {
        ...
        while (!terminado) {
            ...
            if (anteopcion.equals("1")) {
                System.out.print("¿Qué tipo de atracción d"
                		+ "esea crear?\n");
    ...
    public void crearMecanica() {
        ...
        double alturaMaxima = natural("Ingrese la altura máxima para la atracción (m)");
        double alturaMinima = natural("Ingrese la altura mínima para la atracción (m)");
        while (alturaMaxima <= alturaMinima) {
            System.out.println("La altura máxima debe ser mayor que la mínima.");
            alturaMaxima = natural("Ingrese la altura máxima para la atracción (m)");
            alturaMinima = natural("Ingrese la altura mínima para la atracción (m)");
        }

        double pesoMaximo = natural("Ingrese el peso máximo para la atracción (kg)");
        double pesoMinimo = natural("Ingrese la peso mínimo para la atracción (kg)");
        while (pesoMaximo <= pesoMinimo) {
            System.out.println("El peso máximo debe ser mayor que el mínimo.");
            pesoMaximo = natural("Ingrese el peso máximo para la atracción (kg)");
            pesoMinimo = natural("Ingrese la peso mínimo para la atracción (kg)");
        }
    ...
    public void cargarAtraccionConsola() {
        try {
            ...
            if (atraccionCargada != null) {
                System.out.println("Atracción más recientemente cargada: \n");
                atraccionCargada.mostrarAtraccion();
                this.atracciones.add(atraccionCargada);
            }
            
            String input = cadena("¿Desea ver el resto de las atracciones? (s/n)");
            System.out.println("\n");
            if (input.equals("s")) {
            	for (int i = 0; i < atracciones.size(); i++) {
					if (atracciones.get(i) instanceof Atraccion) {
						atracciones.get(i).mostrarAtraccion();
						System.out.println("\n");
					}
				}
            }
    ...
    public void cargarAtraccionPredeterminadaConsola() {
        try {
            Atraccion atraccionCargada = PersistenciaAtracciones.cargarAtraccion("Persistencia.dat");

            if (atraccionCargada != null) {
                System.out.println("Relación de atracción - empleado más recientemente cargada: \n");
                mostrarRelacion(atraccionCargada);
                this.relaciones.add(atraccionCargada);
            }
            
            String input = cadena("¿Desea ver el resto de las relaciones? (s/n)");
            System.out.println("\n");
            if (input.equals("s")) {
            	for (int i = 0; i < relaciones.size(); i++) {
					if (relaciones.get(i) instanceof Atraccion) {
						mostrarRelacion(relaciones.get(i));
						System.out.println("\n");
					}
				}
            }
    ...
    public void mostrarRelacion(Atraccion atraccionCargada) {
    	System.out.println("Atracción: " + atraccionCargada.getNombre());
    	System.out.println("Atracción: " + atraccionCargada.getId());
    	if (atraccionCargada.getEmpleados() != null) {
    		for (int i = 0; i < atraccionCargada.getEmpleados().size(); i++) {
    				System.out.println("Nombre: " + atraccionCargada.getEmpleados().get(i).nombre);
    				System.out.println("ID: " + atraccionCargada.getEmpleados().get(i).getLogin());
					System.out.println("\n");
				}
    	}
    	else {
    		System.out.println("Aún no hay empleados asignados para esta atracción. ");
    	}
    }
