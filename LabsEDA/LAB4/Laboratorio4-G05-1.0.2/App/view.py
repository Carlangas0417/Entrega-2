...
def print_user_position_on_queue(result):
    if result is None:
        print("No se encontró la posición. Seguramente el usuario no quiere leer este libro")
    else:
        print("Posición del usuario en la cola: \n"+str(result))
    
    #Mirar si se puede hacer la conversion de book_id a book_name
...
def main():
    ...
    while working:
        ...
        if int(inputs[0]) == 1:
            ...
            result = logic.get_books_stack_by_user(control, (user_id))
            ...
            result = logic.get_user_position_on_queue(
                control, (user_id), (book_id))
            print_user_position_on_queue(result)
