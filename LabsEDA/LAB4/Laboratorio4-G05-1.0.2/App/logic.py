...
def get_books_stack_by_user(catalog, user_id):
    ...
    readlist = catalog["books_to_read"]["elements"]
    
    for user in range(size):
        if user_id == readlist[user]["user_id"]:
            st.push(books_stack, readlist[user]["book_id"])
    return books_stack
...
def get_user_position_on_queue(catalog, user_id, book_id):
    ...
    while i < size and not encontrado:
        if book_id == readlist[i]["book_id"]:
            q.enqueue(queue, readlist[i]["user_id"])
            position += 1     
            if user_id == sll.last_element(queue):
                encontrado = True  
...
def measure_stack_performance(catalog):
    ...
    for i in range(lt.size(catalog["book_sublist"])):
        book = lt.get_element(catalog["book_sublist"], i)
        st.push(stack, book)
    end_time = get_time()
