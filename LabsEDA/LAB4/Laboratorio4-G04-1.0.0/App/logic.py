...
def get_books_stack_by_user(catalog, user_id):
    ...
    for user in catalog['books_to_read']['elements']:
        if int(user['user_id']) == int(user_id):
            book = catalog['books']['elements'][int(user['book_id'])-1]
...
def get_user_position_on_queue(catalog, user_id, book_id):
    ...
    for user in catalog['books_to_read']['elements']:
        position += 1
        if int(user['user_id']) == int(user_id) and int(user['book_id']) == int(book_id):
            return position
    return None
    """ Creo que esto esta mal, ya que no use las funciones de Cola, aunque eso aumentaria la complejidad de la funcion"""
    """Esta funcion esta hecha teniendo en cuenta que se busca la posicion del usuario para cierto libro
    No para buscar la posicion del usuario sin tener en cuenta el ID del libro, es decir, se debe indicar
    el ID del usuario, asi como el ID del libro adecuado para encontrarlo en la cola"""  
