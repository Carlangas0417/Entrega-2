...
def load_data(control):
    ...
    """
    books, authors, tags, book_tags, total_time, total_memory = logic.load_data(control)
    return books, authors, tags, book_tags, total_time, total_memory
...
def main():
    ...
    while working:
        ...
        if int(inputs[0]) == 1:
            ...
            bk, at, tg, bktg, total_time, total_memory = load_data(control)
            print('Libros cargados: ' + str(bk))
            print('Autores cargados: ' + str(at))
            print('Géneros cargados: ' + str(tg))
            print('Asociación de Géneros a Libros cargados: ' +
                  str(bktg))
            print(f"Tiempo de ejecución: {total_time:.2f} ms")
            print(f"Memoria utilizada: {total_memory:.2f} kB")
