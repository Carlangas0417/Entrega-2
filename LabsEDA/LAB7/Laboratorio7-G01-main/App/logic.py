...
def add_book_author_and_year(catalog, author_name, book):
    ...
    if author_value:
        ...
        new_pub_year_map = lp.new_map(1000, 0.7)  # Crear mapa de años
        books = al.new_list()
        al.add_last(books, book)
        lp.put(new_pub_year_map, pub_year, books)  # Agregar año y libros al mapa de años
        lp.put(books_by_year_author, author_name, new_pub_year_map) 
    return catalog
...
def add_book_tag(catalog, book_tag):
    ...
    if book_tag_value:
        ...
        new_book_tag_list = al.new_list()
        al.add_last(new_book_tag_list, book_tag)
        lp.put(catalog['book_tags'], t['tag_id'], new_book_tag_list)
...
def get_books_by_tag(catalog, tag_name):
    ...
    for i in range(al.size(book_tags)):
        ...
        book = lp.get(catalog['books_by_id'], book_tag['goodreads_book_id'])
        if book:
            al.add_last(books_list, book)
    
    return books_list if al.size(books_list) > 0 else None
...
def get_books_by_author_pub_year(catalog, author_name, pub_year):
    ...
    author_books = lp.get(books_by_year_author, author_name)
    if author_books:
        books_by_year = lp.get(author_books, pub_year)
        
        print(f"Debug: books_by_year = {books_by_year}, type = {type(books_by_year)}")
        
        if books_by_year:
            resultado = books_by_year  
